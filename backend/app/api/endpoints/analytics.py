...
async def track_event(
    ...
    ip_hash = None
    if request.client:
        ip_hash = hashlib.sha256(f"{request.client.host}{ip_salt}".encode()).hexdigest()
...
async def get_analytics_summary(
    ...
    # Top pages
    pages_query = (
        select(AnalyticsEvent.page_url, func.count(AnalyticsEvent.id).label("count"))
    ...
    top_pages = [{"url": url, "count": count} for url, count in pages_result.all()]

    # Top analysis types
    analyses_query = (
        select(AnalyticsEvent.event_data, func.count(AnalyticsEvent.id).label("count"))
        .where(
            and_(
                AnalyticsEvent.created_at >= start_date,
    ...
    for data, count in analyses_result.all():
        try:
            ...
            top_analyses.append(
                {"type": parsed.get("analysis_type", "unknown"), "count": count}
            )
        except:
