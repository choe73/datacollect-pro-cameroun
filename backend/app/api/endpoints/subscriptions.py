...
async def get_quota(
    ...
    if subscription.quota_reset_date:
        if (
            today.month != subscription.quota_reset_date.month
            or today.year != subscription.quota_reset_date.year
        ):
...
async def create_subscription(
    ...
    query = select(Subscription).where(
        and_(Subscription.user_id == current_user.id, Subscription.status == "active")
    )
    result = await db.execute(query)
...
@router.post("/webhook")
async def payment_webhook(request: Request, db: AsyncSession = Depends(get_db)) -> Any:
